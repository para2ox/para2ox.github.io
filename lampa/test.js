...
.card-episode .full-episode::before {
    ...
    z-index: 2;
    box-shadow: inset 2px 2px 1px rgba(255, 255, 255, 0.30);
    background: radial-gradient(circle at center, transparent 58%, rgba(255, 255, 255, 0.22) 75%);
...
.card-episode {
    transition: transform 0.6s cubic-bezier(0.16, 1, 0.3, 1) !important;
        ...
        // ГАРАНТИРОВАННОЕ ОТКРЫТИЕ МЕНЮ ВЫБОРА ПРОФИЛЯ
        profileItem.on('hover:enter', function () {
            if (Lampa.Account && Lampa.Account.open) {
                // Прямой вызов компонента аккаунта Lampa
                Lampa.Account.open();
            } else {
                // Резервный метод через клик, если API недоступно
                $('.head .open--profile').click();
            }
        });
