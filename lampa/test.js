(function () {
    ...
    function startPlugin() {
        ...
        if (allowedEmails.indexOf(email) === -1) {
            console.warn('My Config: Доступ запрещен. Ваш Email:', email);
            return; // Остановка скрипта
        } else {
        ...
        if (Lampa.Utils && Lampa.Utils.putScriptAsync) {
            Lampa.Utils.putScriptAsync([
                ...
                console.log('My Config: Плагины загружены');
            });
        }
        ...
        css += `
            ...
            .card .card__view::before, .card .card__view::before, .card-episode .full-episode::before { content: ''; position: absolute; inset: 0; border: 0 !important; border-radius: 1em; pointer-events: none; opacity: 0; transition: opacity 0.6s cubic-bezier(0.16, 1, 0.3, 1) !important; will-change: opacity; z-index: 2; box-shadow: inset 2px 2px 1px rgba(255, 255, 255, 0.30), inset -2px -2px 2px rgba(255, 255, 255, 0.30); background: radial-gradient(circle at center, transparent 58%, rgba(255, 255, 255, 0.22) 75%, rgba(255, 255, 255, 0.38) 90%), radial-gradient(120% 85% at 18% 10%, rgba(255, 255, 255, 0.35), rgba(255, 255, 255, 0.10) 38%, transparent 62%); mix-blend-mode: screen; }
            .card .card__view::after, .card .card__view::after, .card-episode .full-episode::after { display: none; }
            ...
            .card, .card-episode { transition: transform 0.6s cubic-bezier(0.16, 1, 0.3, 1) !important; will-change: transform; -webkit-animation: none !important; animation: none !important; }
        ...
        // ------------------------------------------------------------------
        // ЧАСТЬ 4: КНОПКИ В МЕНЮ (ПОИСК И ПРОФИЛЬ)
        // ------------------------------------------------------------------
        var menuList = $('.menu .menu__list').eq(0);

        // 4.1 КНОПКА ПОИСКА
        var searchIcon = '<svg xmlns="http://www.w3.org/2000/svg" width="1.2em" height="1.2em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line
