(function () {
    ...
    // ==========================================
    // ЧАСТЬ 1: МГНОВЕННЫЙ ФОН И СТИЛИ (Убивает мигание)
    // ==========================================
    function injectCSS() {
        var css = '';
    ...
    // Внедряем стили мгновенно (не дожидаясь Lampa)
    injectCSS();

    // ==========================================
    // ЧАСТЬ 2: РАННЯЯ ЗАГРУЗКА И ЗАЩИТА
    // ==========================================
    ...
    function initCore() {
        ...
        // Загружаем плагины через стандартный загрузчик ДО завершения лоадера
    ...
    // Перехватываем появление ядра Lampa еще во время лоадера
    var isBooted = false;
    var bootInterval = setInterval(function() {
        if (window.Lampa && window.Lampa.Utils && window.Lampa.Storage && !isBooted) {
            isBooted = true;
    ...
    // Ждем полной отрисовки меню для кнопки
    if (window.appready) {
        appendSearchButton();
    } else {
