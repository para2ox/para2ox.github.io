(function () {
    ...
    // Защита от двойного запуска плагина (полезная фишка от ИИ)
    ...
    function startPlugin() {
        ...
        // Возвращаем нативный метод Lampa, он лучше управляет потоками
        ...
        // Функция для безопасного сохранения (оптимизация: пишем в память, только если значение изменилось)
        ...
        // Настраиваем TorrServer и скачивание торрентов
        ...
        // Возвращаем удобные шаблонные строки для всего CSS
        var css = `
...
/* Эффекты карточек */
.card .card__view::before, .card-episode .full-episode::before, .register::before, .online.selector::before {
    content: '';
    position: absolute;
    inset: 0;
    border: 0 !important;
...
.card, .card-episode, .register, .online.selector {
...
.card.focus .card__view, .card.hover .card__view, .card-episode.focus .full-episode, .register.focus, .online.selector.focus {
...
.card .card__view, .card-episode .full-episode, .register, .online.selector {
    animation: none !important;
    margin-bottom: 1em;
}
...
`;
        ...
        function injectRouteGuard() {
            ...
            // Сохраняем оригинальную функцию навигации
            var originalPush = Lampa.Activity.push;
            
            // Временно переопределяем её
            Lampa.Activity.push = function (params) {
                var active = Lampa.Activity.active();
                // Если мы сейчас в карточке фильма, а приложение пытается швырнуть нас на главную - блокируем
                if (active && active.component === 'full' && (params.component === 'main' || params.component === 'surs')) {
                    console.log('My Config: Заблокирован случайный выброс на главную страницу');
                    return false; 
                }
                return originalPush.apply(this, arguments);
            };

            // Через 4 секунды снимаем блокировку и возвращаем всё как было (самоочистка памяти)
            setTimeout(function() {
                Lampa.Activity.push = originalPush;
                console.log('My Config: Стартовый щит навигации отключен');
            }, 4000);
        }
        ...
        // Защита от добавления дублей кнопки (полезная штука от ИИ)
    ...
    // Запуск скрипта (Возвращаем нативный ивентер Lampa вместо тяжелых интервалов)
    if (window.appready) {
        startPlugin();
    } else {
        // Микро-интервал только для того, чтобы дождаться инициализации ядра Lampa.Listener
