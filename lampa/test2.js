(function () {
    ...
    var plugin_id = 'my_super_config_v3';
    if (window[plugin_id]) return;
    window[plugin_id] = true;

    // Функция объявлена в глобальной области видимости плагина (никаких try/catch вокруг нее)
    function startPlugin() {
        ...
        function safeSetConfig(key, value) {
            if (window.Lampa && window.Lampa.Storage && String(window.Lampa.Storage.get(key)) !== String(value)) {
                window.Lampa.Storage.set(key, value);
            }
        }
        ...
        // --- ЧАСТЬ 3: ES5 СОВМЕСТИМАЯ CSS ИНЪЕКЦИЯ ---
        var css = '';
        ...
        function injectRouteGuard() {
            if (!window.Lampa || !window.Lampa.Activity || !window.Lampa.Activity.push) return;
            var originalPush = window.Lampa.Activity.push;
            ...
            setTimeout(function() {
                if (window.Lampa.Activity.push !== originalPush) {
                    window.Lampa.Activity.push = originalPush;
                }
            }, 4000);
        }
        injectRouteGuard();
        ...
        searchItem.on('hover:enter', function () {
            ...
            else if (window.Lampa && window.Lampa.Search) window.Lampa.Search.open();
        });

        // В jQuery length === 0 означает, что элемент не найден
        if ($('.menu .menu__list li[data-action="search_button"]').length === 0) {
            $('.menu .menu__list').eq(0).append(searchItem);
        }
    }

    // --- ОРИГИНАЛЬНАЯ НАДЕЖНАЯ ИНИЦИАЛИЗАЦИЯ ---
    if (window.appready) {
        startPlugin();
        ...
        // Микро-таймер на случай, если наш плагин загрузился раньше самого ядра Lampa
